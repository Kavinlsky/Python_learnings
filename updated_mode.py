...
class PDFViewer():
    ...
    def __init__(self,root):
        ...
        self.canvas_left = tk.Canvas(self.root, bg='gray', width=400, height=710)
        self.canvas_left.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        self.canvas_right = tk.Canvas(self.root, bg='#FAF0E0', width=760, height=710)
        ...
        self.file = Menu(self.root, tearoff=0)
        ...
        self.edit = Menu(self.root)
        self.edit = Menu(self.root, tearoff=0)
        self.menubar.add_cascade(label='Edit', menu=self.edit)
        self.edit.add_command(label='Clear', command=self.clear_text)

        self.process_menu = Menu(self.root)
        self.process_menu = Menu(self.root, tearoff=0)
        self.menubar.add_cascade(label="Process", command=self.process)

        self.go_to_page = Menu(self.root)
        self.go_to_page = Menu(self.root, tearoff=0)
    ...
    def buttons(self):
        ...
        # self.prev_button = tk.Button(self.root, text="Previous Page", command=self.show_prev_page)
        # self.prev_button.pack(pady=5)
        #
        # self.next_button = tk.Button(self.root, text="Next Page", command=self.show_next_page)
        # self.next_button.pack(pady=5)

        # self.mode_switch = ttk.Checkbutton(self.root, text="Mode", style="Switch.TCheckbutton", command=self.toggle_mode)
