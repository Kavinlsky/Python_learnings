...
class ImageCropApp:
    ...
    def __init__(self, root):
        ...
        #     file='logo.png'))  # self.root.iconbitmap("D:\KavinKumar-6204\Kavin-Python\pythonProject2\OCR_projects__\Pdf_processing/tkinter_app_\Application\logo.ico")
        #
    ...
    def screen_properties(self):
        ...
        self.automate = tk.Menu(root)
        self.automate = tk.Menu(root, tearoff=0)
        self.menubar.add_cascade(label='Automate', command=self.temp_process_trigger)

        root.config(menu=self.menubar)

    def windows_widgets(self):
        # separator_line = self.canvas_right.create_line(100, 20, 500, 20, fill="gray")
        self.title_of_right_canvas = tk.Label(self.canvas_right, text="Processed Text", font=("Helvetica", 16),
        ...
        self.submit_button = tk.Button(self.canvas_right, text="Submit", command=self.get_result_from_text_box)
        self.submit_button.grid(row=13, column=1, padx=10, pady=10, sticky='nsw')

        self.root.bind("<Control-j>", self.temp_process)
        self.root.bind("<Control-z>", self.undo_del_rectangle)
    ...
    def export_to_csv(self):
        if self.data:
            with open("data.csv", mode="w", newline="") as file:
                ...
                writer.writerow(
                    ["File Name", "Name", "Organization", "Date", "Gender", "Date_of_Birth", "Phone Number", "Email",
                     "Address", "Medicines"])
    ...
    def save_co_ordinates(self):
        ...
        self.save_text_file_path = filedialog.asksaveasfilename(defaultextension=".txt",
                                                                filetypes=[("Text Files", "*.txt")],
                                                                initialfile=f'{self.text_file_name}_co-ordinates.txt')
    ...
    def open_folder(self):
        ...
        if self.folder_path:
            self.image_files = [os.path.join(self.folder_path, file) for file in os.listdir(self.folder_path) if
    ...
    def temp_process(self, event):
        self.show_next_image()
        self.root.after(20000, self.execute_hotkey)

    def temp_process_trigger(self):
        self.show_next_image()
        self.root.after(20000, self.execute_hotkey)
    ...
    def undo_del_rectangle(self, event):
